...
def show():
    ...
    # Replace IDs with names in df
    df['district_name'] = df['district_id'].map(dist_map)
    df['department_name'] = df['department_id'].map(dept_map)
    df['theme_name'] = df['thematic_category_id'].map(theme_map)
    ...
    if report_type == "District-wise Convergence Report":
        ...
        st.dataframe(fin_df)
        fig = px.bar(fin_df, x='district_name', y=['Dept_Fund', 'VBG_Fund'], title="Financial Convergence by District", barmode='stack')
        ...
        if not tech_df.empty:
            tech_summary = tech_df.groupby('department_name').agg(Count=('id', 'count')).reset_index()
            ...
            st.info("No technical convergence activities.")

    elif report_type == "Scheme Performance Report":
        st.subheader("Scheme-wise Performance")
        # Group by activity_description or department
        ...
        scheme_perf['Financial%'] = (scheme_perf['Financial_Ach'] / scheme_perf['Target'].replace(0, 1)) * 100  # simplified
        ...
        pd_df = df.groupby('district_name').agg(
        ...
        st.dataframe(pd_df)
        fig = px.bar(pd_df, x='district_name', y=['Expected', 'Actual'], barmode='group', title="Persondays by District")
        st.plotly_chart(fig, use_container_width=True)
        ...
        delayed = df[df['current_status'].isin(['Planned', 'Approved', 'Delayed']) | (df['delay_days'] > 0)]
        if not delayed.empty:
            st.dataframe(delayed[['id', 'activity_description', 'district_name', 'department_name', 'current_status', 'delay_days']])
            excel = dataframe_to_excel(delayed, "Delayed_Activities")
            st.download_button("📥 Download Excel", excel, "delayed_activities.xlsx")
        else:
        ...
        meetings = supabase.table("meetings").select("*").eq("meeting_type", "District").execute().data
        if meetings:
        ...
        block_summary = df.groupby(['district_name', 'block_id']).agg(
            Activities=('id', 'count'),
            Target=('desired_target', 'sum'),
        ...
        block_summary['block_name'] = block_summary['block_id'].map(block_map)
        ...
        st.dataframe(df)  # full data with names

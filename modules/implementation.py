...
def safe_parse_date(date_val):
  ...
  Returns the exact date if it exists, otherwise returns None to prevent
...
def show():
  ...
  st.markdown(
      "<h1 style='color: #1F77B4;'>🚀 Implementation & Target Monitoring</h1>",
      unsafe_allow_html=True,
  )
  st.markdown("---")
  ...
  blocks = (
      supabase.table("blocks").select("id,block_name,district_id").execute().data
  )
  ...
  with tab1:
    ...
    if role == "department":
      ...
      t_dists = (
          districts
          if role == "superadmin"
          else [d for d in districts if d["id"] == user.get("district_id")]
      )
    ...
    with col_t2:
      ...
      if role == "block":
        st.info(
            "Target setting is managed at the District/Department level. You"
            " can view targets on the left."
        )
      else:
        with st.form("target_form"):
          if role == "department":
            ...
            dept_sel = st.selectbox(
                "Department",
                list(t_dept_dict.keys()) if t_dept_dict else ["None"],
            )
            dist_sel = st.selectbox(
                "District",
                list(t_dist_dict.keys()) if t_dist_dict else ["None"],
            )

          activity = st.text_input("Activity (optional)")
          col_tf1, col_tf2 = st.columns(2)
          desired_target = col_tf1.number_input(
              "Desired Target for FY", min_value=0, value=0
          )
          asset_count = col_tf2.number_input(
              "Number of assets/works", min_value=0, value=0
          )

          annual_plan_scope = st.text_area("Scope under Annual Plan")

          col_tf3, col_tf4 = st.columns(2)
          dept_fund = col_tf3.number_input(
              "Dept Fund (₹ Lakhs)", min_value=0.0, format="%.2f"
          )
          vbg_fund = col_tf4.number_input(
              "VB-G Fund (₹ Lakhs)", min_value=0.0, format="%.2f"
          )

          expected_persondays = st.number_input(
              "Expected Persondays", min_value=0, value=0
          )

          submitted_target = st.form_submit_button(
              "Save Target", type="primary", use_container_width=True
          )
          if submitted_target and dept_sel != "None" and dist_sel != "None":
            ...
            existing = (
                supabase.table("department_targets")
                .select("id")
            ...
            if existing:
              ...
              log_action(
                  user.get("id"), f"CREATE department_targets {new_target_id}"
              )
    ...
    with col_t1:
      ...
      if role == "department":
        query_t = query_t.eq("department_id", user.get("department_id")).eq(
      ...
      if data_t:
        ...
        st.dataframe(
            df_t[disp_cols], use_container_width=True, hide_index=True
        )
  ...
  with tab2:
    ...
    st.caption(
        "Update the on-ground reality of specific schemes. MIS Code is mandatory"
    ...
    if role == "district":
      ...
      query_reg = query_reg.eq("department_id", user["department_id"]).eq(
    ...
    if not activities:
      ...
      activity_map = {
          a["id"]: (
              f"{a.get('activity_code', a.get('id', ''))} -"
              f" {a.get('activity_description', 'Unnamed Activity')[:60]}..."
          )
          for a in activities
      }

      selected_act_id = st.selectbox(
          "Select Convergence Activity to Update",
          options=list(activity_map.keys()),
          format_func=lambda x: activity_map[x],
      )
      selected_activity = next(
          (a for a in activities if a["id"] == selected_act_id), None
      )

      if selected_activity:
        # Proper HTML rendering with unsafe_allow_html=True
        st.markdown(
            f"**Current Status:** <span style='color:"
            f" #E67E22;'>{selected_activity.get('current_status', 'Planned')}</span>"
            " | **Source:**"
            f" <code>{selected_activity.get('origin_source', 'Normative / Routine')}</code>",
            unsafe_allow_html=True,
        )
        ...
        with st.form("update_progress_form"):
          ...
          new_status = col_p1.selectbox(
              "New Status", status_options, index=status_idx
          )
          phys_ach = col_p2.slider(
              "Physical Achievement (%)",
              min_value=0,
              max_value=100,
              value=int(
                  float(
                      selected_activity.get("physical_achievement", 0.0) or 0.0
                  )
              ),
          )
          ...
          mis_code_val = col_p3.text_input(
              "MIS Code (e.g. 3206011003/RS...)",
              value=selected_activity.get("mis_code", "") or "",
          )
          fin_ach = col_p4.number_input(
              "Financial Achievement (₹ Lakhs)",
              min_value=0.0,
              value=float(
                  selected_activity.get("financial_achievement", 0.0) or 0.0
              ),
          )

          persondays_gen = st.number_input(
              "Persondays Generated (Cumulative)",
              min_value=0,
              value=int(
                  selected_activity.get("persondays_generated", 0) or 0
              ),
          )
          ...
          db_start = safe_parse_date(
              selected_activity.get("actual_start_date")
          )
          db_exp = safe_parse_date(
              selected_activity.get("expected_completion_date")
          )
          db_act = safe_parse_date(
              selected_activity.get("actual_completion_date")
          )
          ...
          act_date = col_p7.date_input(
              "Actual Completion (If done)", value=db_act
          )

          remarks = st.text_area(
              "Remarks / Blockages",
              value=selected_activity.get("remarks", "") or "",
          )

          submitted_prog = st.form_submit_button(
              "Save Progress", type="primary", use_container_width=True
          )

          if submitted_prog:
            if (
                new_status in ["Under Implementation", "Completed"]
                and not mis_code_val.strip()
            ):
              st.error(
                  "⚠️ **Validation Error:** MIS Code is strictly mandatory"
                  " when moving a scheme to 'Under Implementation' or"
                  " 'Completed'. Please enter the valid MIS Code from the"
                  " central portal to proceed."
              )
            else:
              update_data = {
              ...
              try:
                ...
                log_action(
                    user.get("id"),
                    f"UPDATE convergence_register {selected_act_id}",
                )

                st.success(
                    "✅ Progress and MIS mapping updated successfully!"
                )
                st.rerun()
        ...
        try:
          history_query = (
              supabase.table("progress_updates")
              .select("*")
              .eq("convergence_id", selected_act_id)
  ...
  with tab3:
    ...
    st.caption(
        "View and fulfill the Action Points assigned to your department across"
        " all District and Block meetings."
    )

    ap_query = (
        supabase.table("meeting_action_points")
        .select(
            "id, meeting_id, department_id, priority, linkage_type,"
            " action_point, target, deadline, status, remarks"
        )
        .execute()
        .data
    )
    ...
    if ap_query:
      ...
      if not df_ap.empty:
        ...
        df_ap["Meeting Context"] = df_ap["meeting_id"].map(
            lambda x: (
                f"{m_map.get(x, {}).get('meeting_type', 'Unknown')} Level"
                f" ({m_map.get(x, {}).get('meeting_date', 'Unknown')})"
            )
        )

        pending_ap = df_ap[
            ~df_ap["status"].isin(["Completed", "Dropped"])
        ].copy()

        if not pending_ap.empty:
          pending_ap["deadline"] = pd.to_datetime(pending_ap["deadline"])
          pending_ap["Days Left"] = (
              pending_ap["deadline"] - pd.to_datetime(date.today())
          ).dt.days
          pending_ap["Linkage"] = pending_ap.get(
              "linkage_type", "Normative / Routine"
          )
          ...
          st.dataframe(
              pending_ap[disp_cols].sort_values("Days Left"),
              use_container_width=True,
              hide_index=True,
          )
          ...
          with st.form("sync_atr_form"):
            ...
            sync_id = col_s1.selectbox(
                "Select Resolution ID",
                pending_ap["id"].tolist(),
                format_func=lambda x: (
                    f"[{pending_ap[pending_ap['id'] == x]['Meeting Context'].values[0]}]"
                    f" {pending_ap[pending_ap['id'] == x]['action_point'].values[0][:40]}..."
                ),
            )

            # ADDED 'Not Feasible' option to seamlessly loop back to the meeting agenda
            sync_status = col_s2.selectbox(
                "New Status",
                [
                    "Under Process",
                    "Approved",
            ...
            sync_remarks = st.text_area(
                "Implementation Outcome / Remarks (If Not Feasible, state the"
                " reason clearly)"
            )

            submitted_sync = st.form_submit_button(
                "Sync Progress to Meeting Tracker"
            )

            if submitted_sync:
              if (
                  sync_status == "Not Feasible (Requires Review)"
                  and not sync_remarks.strip()
              ):
                st.error(
                    "⚠️ **Validation Error:** You must provide a clear reason in"
                    " 'Remarks' when flagging an activity as Not Feasible so the"
                    " Chairperson can review it."
                )
              else:
                ...
                # Fixed log_action signature
                log_action(
                    user.get("id"), f"UPDATE meeting_action_points {sync_id}"
                )

                st.success(
                    "✅ Meeting ATR Updated! If flagged as Not Feasible, it has"
                    " been added to the next meeting's agenda."
                )
                st.rerun()
        else:
          st.success(
              "🎉 All meeting commitments have been completed or closed!"
          )
      else:

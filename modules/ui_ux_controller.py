...
def init_ui_state():
    """Initialize default UI settings in session state if they don't exist."""
    defaults = {
...
def show():
    # Only Super Admin can access this powerful controller
    ...
    # ==========================================
    # 3-COLUMN LAYOUT (Matching the Specification)
    # ==========================================
    # Ratios: Design Menu (2), Live Preview (5), Properties (2)
    col_design, col_preview, col_prop = st.columns([2, 5, 2.5])

    # ------------------------------------------
    # COLUMN 1: DESIGN MENU
    # ------------------------------------------
    with col_design:
    ...
    # ------------------------------------------
    # COLUMN 3: PROPERTIES PANEL (Dynamic)
    # ------------------------------------------
    with col_prop:
    ...
    with col_preview:
        ...
        # Dynamic CSS based on Properties Panel choices
        ...
        # Display a mock chart preview colored dynamically
    ...
    with action_col3:
        if st.button("🚀 Publish to Live", type="primary", use_container_width=True):
            # In a real database scenario, you would save these to a 'system_settings' table here

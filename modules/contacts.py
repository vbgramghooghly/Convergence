...
def show():
    ...
    # If role is department, restrict viewing strictly to their department/wing
    if role == "department":
    ...
    # ==============================================================
    # TAB 2: DIRECTORY MANAGER (Restricted for Departments)
    # ==============================================================
    with tab2:
        existing_record = {}
        ...
        if role in ["superadmin", "district", "block"]:
            ...
            if action == "✏️ Edit Existing Official":
                ...
                if contact_list:
                    contact_opts = {
                        c["id"]: f"{c['full_name']} - {c.get('designations', {}).get('designation_name', 'Unknown')}"
            ...
            st.subheader("🔄 Department Contact Personnel Management")
            st.caption(
                f"Manage contact personnel specifically for **{dept_map.get(user.get('department_id'), 'Your Department')}**."
            )

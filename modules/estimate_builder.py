...
def show():
    ...
    def get_spec_unit(code):
        if not code or code == 'LS' or df_specs.empty: return ""
        try: return str(df_specs[df_specs['spec_code'] == code]['final_unit'].iloc[0])
        except: return ""
    ...
    def add_lumpsum_heading():
        st.session_state['estimate_data'].append({
            ...
            'items': [{
                'id': str(uuid.uuid4()), 'spec_code': 'LS', 'input_type': 'Lump Sum (LS)',
                'L': 1.0, 'B': 1.0, 'D': 1.0, 'qty': 1.0,
                'ls_desc': 'Contingency / Site Clearance', 'ls_amount': 0.0,
                'material_gsts': {}, 'last_spec': None
            }]
        })
    ...
    def calculate_item_cost(spec_code, final_qty, material_gsts_dict):
        ...
        try: base_qty = float(df_specs[df_specs['spec_code'] == spec_code]['base_qty'].iloc[0])
        ...
        if recipe.empty: return 0.0, 0.0, 0.0, 0.0, []
        ...
        for _, row in pure_materials.iterrows():
            ...
            material_breakdown.append({
                'lmr_code': mat_code, 'description': row['description'],
                'multiplier': row['multiplier'], 'rate': row['rate'],
                'amount': row['amount'], 'gst_rate': gst_pct, 'gst_amount': gst_amt
            })
        ...
        for heading in estimate_data:
            for item in heading.get('items', []):
                if item['input_type'] == "Lump Sum (LS)":
                    ...
                    item_details.append({
                        'heading': heading['title'], 'spec_code': 'LS', 'description': item.get('ls_desc', 'Lump Sum'),
                        'qty': 1, 'unit': 'LS', 'unskilled': 0.0, 'skilled': 0.0,
                        'material': ls_amt, 'gst': 0.0, 'total': ls_amt, 'materials': []
                    })
                else:
                    ...
                    total_material += mat
                    total_gst += gst
                    try: spec_desc = df_specs[df_specs['spec_code'] == spec]['description'].iloc[0]
                    except: spec_desc = spec
                    item_details.append({
                        'heading': heading['title'], 'spec_code': spec, 'description': spec_desc,
                        'qty': qty, 'unit': get_spec_unit(spec), 'unskilled': unsk, 'skilled': skilled,
                        'material': mat, 'gst': gst, 'total': unsk + skilled + mat + gst, 'materials': mat_break
                    })

        grand_total = total_unskilled + total_skilled + total_material + total_gst
        return {
            'unskilled': total_unskilled, 'skilled': total_skilled, 'material': total_material,
            'gst': total_gst, 'grand_total': grand_total, 'item_details': item_details
        }
    ...
    def generate_print_html(work_name, work_type, district_name, watermark, totals):
        ...
        for detail in totals['item_details']:
            html += f"""
            <tr>
                <td>{detail['heading']}</td><td>{detail['description']}</td><td>{detail['spec_code']}</td>
                <td>{detail['qty']:.2f}</td><td>{detail['unit']}</td><td>₹{detail['unskilled']:,.2f}</td>
                <td>₹{detail['skilled']:,.2f}</td><td>₹{detail['material']:,.2f}</td><td>₹{detail['gst']:,.2f}</td>
        ...
        html += f"""
            ...
            <tfoot>
                <tr><th colspan="5">Totals</th>
                    <th>₹{totals['unskilled']:,.2f}</th><th>₹{totals['skilled']:,.2f}</th>
                    <th>₹{totals['material']:,.2f}</th><th>₹{totals['gst']:,.2f}</th>
    ...
    with top_col1: st.info(f"📍 **Active District:** {active_district_name}")
    with top_col2:
        view_mode = st.radio("Display Mode", ["Edit Workspace", "View Appendix-I Report"], horizontal=True, label_visibility="collapsed")
    with top_col3:
            ...
            for h_idx, heading in enumerate(st.session_state['estimate_data']):
                with st.expander(f"📁 {heading['title']}", expanded=True):
                    ...
                    for i_idx, item in enumerate(heading['items']):
                        ...
                        if 'material_gsts' not in item: item['material_gsts'] = {}
                        if 'last_spec' not in item: item['last_spec'] = None

                        c1, c2, c3, c4, c5 = st.columns([3, 2, 3, 2, 1])
                        item['input_type'] = c2.radio("Input Type", ["Direct Quantity", "L × B × D", "Lump Sum (LS)"], key=f"type_{item['id']}", horizontal=True, label_visibility="collapsed")
                        ...
                        if item['input_type'] != "Lump Sum (LS)":
                            item['spec_code'] = c1.selectbox(
                                ...
                                key=f"spec_{item['id']}", format_func=format_spec_dropdown
                            )
                            ...
                            if 'spec_norm' not in df_matrix.columns: df_matrix['spec_norm'] = df_matrix['spec_code'].str.strip().str.upper()
                            recipe = df_matrix[df_matrix['spec_norm'] == spec_norm].copy()

                            if recipe.empty:
                                c4.markdown(f"<div style='font-size:12px; padding:5px; background-color:#fff3cd; border-radius:5px; border:1px solid #ffeeba;'><b style='color:#856404;'>⚠️ No consumption data for this spec</b><br>Please add entries in the <i>Consumption Matrix</i> (Super Admin).</div>", unsafe_allow_html=True)
                                ...
                                unskilled_amt, skilled_amt, mat_amt, item_gst, mat_breakdown = calculate_item_cost(item['spec_code'], item['qty'], item['material_gsts'])

                                c4.markdown(f"<div style='font-size:12px; line-height:1.2; padding:5px; background-color:#f0f2f6; border-radius:5px;'><b style='color:#0056b3;'>Qty: {item['qty']:.2f} {active_unit}</b><br><br><b>Unskilled (0115):</b> ₹{unskilled_amt:,.2f}<br><b>Skilled/Semi:</b> ₹{skilled_amt:,.2f}<br><b>Pure Material:</b> ₹{mat_amt:,.2f}<br><b style='color:#d32f2f;'>GST (Aggregated):</b> ₹{item_gst:,.2f}</div>", unsafe_allow_html=True)
                                ...
                                if pure_mats:
                                    with st.expander("⚙️ Configure Material GST Slabs", expanded=False):
                                        ...
                                        for idx, m_code in enumerate(pure_mats):
                                            ...
                                            item['material_gsts'][m_code] = cols[col_idx].selectbox(f"{m_code} - {m_desc[:15]}...", [0, 5, 12, 18, 28], index=[0, 5, 12, 18, 28].index(item['material_gsts'].get(m_code, 18)), key=f"gst_{item['id']}_{m_code}", format_func=lambda x: f"{x}%")
                        else:
                            ...
                            c4.markdown(f"<div style='font-size:12px; padding:5px; background-color:#e8f4f8; border-radius:5px;'><b style='color:#0056b3;'>Qty: 1 (LS)</b><br><br><b>Lump Sum Amount:</b> ₹{item['ls_amount']:,.2f}<br><b style='color:#d32f2f;'>GST Exempt (0%)</b></div>", unsafe_allow_html=True)
                            unskilled_amt, skilled_amt, mat_amt, item_gst, item['spec_code'], item['qty'] = 0.0, 0.0, item['ls_amount'], 0.0, 'LS', 1
        ...
        for detail in totals['item_details']:
            if detail['materials']:
                with st.expander(f"{detail['spec_code']} - {detail['description']}"):
                    st.dataframe(pd.DataFrame(detail['materials'])[['lmr_code', 'description', 'multiplier', 'rate', 'amount', 'gst_rate', 'gst_amount']], hide_index=True, use_container_width=True)
        st.caption("DPR Preview (materials & GST)")

    # ---------- TEMPLATES SECTION ----------
    st.markdown("---")
    with st.expander("💾 Approved Templates", expanded=False):
        try: saved_templates = supabase.table("saved_templates").select("id, template_name, estimate_data").execute().data
        except: saved_templates = []
            ...
            for t in saved_templates:
                ...
                if st.session_state.get('authenticated') and st.session_state.get('role') in ['SuperAdmin', 'StateAdmin', 'DistrictAdmin']:
                    if col3.button("🗑️", key=f"dl_{t['id']}", use_container_width=True):
                        ...
                        st.rerun()
        else: st.caption("No templates saved yet.")

        if st.session_state.get('authenticated') and st.session_state.get('role') in ['SuperAdmin', 'StateAdmin', 'DistrictAdmin']:
            if st.button("➕ Save Current as New Template", use_container_width=True):
                if not st.session_state['estimate_data']: st.error("Cannot save an empty estimate!")
                else:
                    user_name = st.session_state.get('full_name', 'Engineer')
                    supabase.table("saved_templates").insert({"owner_name": user_name, "template_name": f"Template {len(saved_templates) + 1} ({user_name})", "estimate_data": st.session_state['estimate_data']}).execute()

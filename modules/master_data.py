...
def show():
    ...
    for w in wings:
        ...
        dept_options.append({"label": f"{parent_name} ➔ {w['wing_name']} [{w['entity_type']}]", "dept_id": w['department_id'], "wing_id": w['id']})
    ...
    if st.button("Update User Profile", type="primary"):
        if new_role == 'department' and not new_dept_id:
            ...
            try:
                resp = supabase.table("users").update(updates).eq("id", selected_uid).execute()
                if resp.count and resp.count > 0:
                    try: log_action(user.get('id'), f"UPDATE users {selected_uid}")
                    except: pass
    ...
    if submitted:
        ...
        try:
            ...
            try:
                ...
                try: log_action(user.get('id'), f"CREATE users {new_uuid}")
                except: pass 

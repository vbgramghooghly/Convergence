...
def show():
    ...
    if not df_records.empty:
        ...
        total_pdays = pd.to_numeric(df_records.get('expected_persondays', 0), errors='coerce').sum()
        active_count = len(df_records[df_records.get('current_status', '').isin(['Planned', 'Approved', 'Under Implementation'])])
        completed_count = len(df_records[df_records.get('current_status', '') == 'Completed'])

        c1.metric("Total Works Registered", len(df_records))

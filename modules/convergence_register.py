...
def display_register(df, maps):
    ...
    def extract_base_activity(desc):
        if not desc: return ""
        if " at " in desc: return desc.split(" at ", 1)[0].strip()
        return desc.strip()
    ...
    df_display.rename(columns={"activity_description": "Work Name", "geo_location": "Location Details", "origin_source": "Source", "convergence_type": "Convergence Type", "current_status": "Status", "total_converged_fund": "Total Fund (₹ Lakhs)"}, inplace=True)
    display_cols = ["FY", "District", "Block", "Department", "Wing", "Base Activity", "Work Name", "Location Details", "Source", "Convergence Type", "Status", "Total Fund (₹ Lakhs)"]
...
def render_scheme_convergence_section(defaults, key_prefix=""):
    ...
    conv_choice = st.radio("Convergence with Own Departmental Scheme / Fund?", options=["No", "Yes"], index=0 if not defaults.get("convergence") else 1, key=f"{key_prefix}_conv_choice")
    scheme_name = ""
    if conv_choice == "Yes": scheme_name = st.text_input("Name of Departmental Scheme / Fund *", value=defaults.get("scheme_name", ""), key=f"{key_prefix}_scheme_name")
    ...
    annual_plan_status = st.selectbox("Included in Department's Own Annual Plan?", options=status_options, index=status_options.index(default_status), key=f"{key_prefix}_annual_status")
    scheme_remarks = st.text_area("Departmental Scheme / Annual Plan Remarks (Optional)", value=defaults.get("scheme_remarks", ""), key=f"{key_prefix}_scheme_remarks")
    return {"convergence": conv_choice == "Yes", "scheme_name": scheme_name.strip() if scheme_name else None, "annual_plan_status": annual_plan_status, "scheme_remarks": scheme_remarks.strip() if scheme_remarks else None}

def edit_delete_section(records, maps, supabase, user, master):
    if user["role"] not in ["superadmin", "district"]: return
    if not records: st.info("No records available to manage."); return
    ...
    with st.container(border=True):
        ...
        if sel_block != "All": gp_names = sorted(HOOGHLY_GPS.get(str(sel_block).upper().strip(), []))
        else:
            gps = set()
            for r in records:
                loc = r.get("geo_location", "")
                if "GP:" in loc:
                    try: gps.add(loc.split("GP:")[1].split("|")[0].strip())
                    except: pass
    ...
    if sel_block != "All": filtered_records = [r for r in filtered_records if r.get("block_id") == maps["block_map"].get(sel_block)]
    if sel_dept != "All": filtered_records = [r for r in filtered_records if r.get("department_id") == maps["dept_map"].get(sel_dept)]
    if sel_gp != "All": filtered_records = [r for r in filtered_records if sel_gp in r.get("geo_location", "")]
    if search_text: filtered_records = [r for r in filtered_records if search_text.lower() in r.get("activity_description", "").lower()]
    if not filtered_records: st.warning("No activities match the selected filters."); return
    st.markdown("---")
    st.markdown("#### 🛠️ Modify Selected Activity")
    display_options = {r["id"]: f"{r['activity_description']} - {maps['dept_reverse'].get(r['department_id'], 'Unknown')} (₹{r.get('total_converged_fund', 0)} L)" for r in filtered_records}
    ...
    with st.form("edit_conv_form"):
        ...
        new_status = col_e1.selectbox("Status", STATUS_OPTIONS, index=STATUS_OPTIONS.index(rec.get("current_status", "Planned")) if rec.get("current_status", "Planned") in STATUS_OPTIONS else 0)
        new_conv_type = col_e2.selectbox("Convergence Type", CONVERGENCE_TYPES, index=CONVERGENCE_TYPES.index(rec.get("convergence_type", CONVERGENCE_TYPES[0])) if rec.get("convergence_type", CONVERGENCE_TYPES[0]) in CONVERGENCE_TYPES else 0)
        curr_pia = rec.get("pia_type", "Select PIA")
        if curr_pia not in PIA_OPTIONS: curr_pia = "Select PIA"
        new_pia = st.selectbox("Project Implementing Agency (PIA)*", PIA_OPTIONS, index=PIA_OPTIONS.index(curr_pia))
        ...
        wing_labels, wing_ids = [label for label, _ in wing_choices], [wid for _, wid in wing_choices]
        current_wing_id = rec.get("wing_id")
        default_idx = wing_ids.index(current_wing_id) if current_wing_id in wing_ids else 0
        selected_wing_label = st.selectbox("Entering Wing / Scheme Source", options=wing_labels, index=default_idx)
        ...
        default_index = 0
        if current_base_act in valid_act_names_edit: default_index = valid_act_names_edit.index(current_base_act)
        ...
        st.markdown("---")
        defaults = {"convergence": rec.get("department_scheme_convergence", False), "scheme_name": rec.get("department_scheme_name", "") or "", "annual_plan_status": rec.get("department_annual_plan_status", "Not Confirmed"), "scheme_remarks": rec.get("department_scheme_remarks", "") or ""}
        ...
        if st.form_submit_button("Commit Changes", type="primary"):
            errors = []
            if new_conv_type == "Technical Convergence (Zero Fund/NOC)": new_d_fund = new_v_fund = 0.0
            if new_pia == "Select PIA": errors.append("⚠️ Please select a valid PIA.")
            elif scheme_data["convergence"] and not scheme_data["scheme_name"]: errors.append("⚠️ Scheme name mandatory.")
            elif new_conv_type != "Technical Convergence (Zero Fund/NOC)" and new_d_fund == 0.0 and new_v_fund == 0.0: errors.append("⚠️ Financial Convergence requires a Fund amount > 0.")
            elif new_pd <= 0: errors.append("⚠️ Expected Persondays must be > 0.")
            if errors:
                ...
                update_payload = {
                    ...
                    "pia_type": new_pia,
                    "wing_id": new_wing_id,
                    "activity_id": selected_act_edit_rec["id"] if selected_act_edit_rec else None, # <-- ADDED activity_id for Edit mode
...
def show():
    ...
    if not master["fys"]: st.error("⚠️ No active Financial Years found in the database."); st.stop()
    ...
    tab_list = ["📋 Master Work Register", "➕ Add New Activity"]
    if role in ["superadmin", "district"]: tab_list.append("🛠️ Manage / Amend Activity")
    tabs = st.tabs(tab_list)

    with tabs[0]: display_register(df_records, maps)
    with tabs[1]:
        st.markdown("#### ➕ Register Individual Convergence Activity")
        with st.container(border=True):
            col1, col2 = st.columns(2)
            selected_fy_id = col1.selectbox("Financial Year*", options=list(maps["fy_reverse"].keys()), format_func=lambda x: maps["fy_reverse"][x])
            ...
            if role == "department":
                selected_opt = next((opt for opt in dept_options if opt['dept_id'] == user.get("department_id") and opt['wing_id'] == user.get("wing_id")), None)
                if selected_opt: sel_dept_label = col2.selectbox("Department / Wing*", [selected_opt["label"]], disabled=True)
                else:
                ...
                sel_dept_label = col2.selectbox("Department / Wing*", dept_labels)
                selected_opt = next(opt for opt in dept_options if opt['label'] == sel_dept_label)
            selected_dept_id, selected_wing_id = selected_opt['dept_id'], selected_opt['wing_id']
            ...
            else: sel_dist = col1.selectbox("District*", list(maps["dist_map"].keys()))
            ...
            if role == "block": sel_block = col2.selectbox("Block*", [next(b["block_name"] for b in master["blocks"] if b["id"] == user["block_id"])], disabled=True)
            else: sel_block = col2.selectbox("Block*", ["Select Block"] + filtered_blocks)
            ...
            if has_add_gp == "Yes": additional_gp = col_gp2.selectbox("Additional GP Name", gp_options); add_gp_portion = col_gp3.text_input("Portion in Addl. GP")
            ...
            theme_id = None
            if not valid_act_names: sel_act_name = col_act1.selectbox("Base Activity*", ["No activities available"], disabled=True)
            else:
            ...
            with col_wn: st.text_input("Work Name (Auto-generated)", value=auto_desc, disabled=True)
            final_work_name = auto_desc
            ...
            if role == "district": origin_options = ["District Annual Action Plan", f"District Meeting ({maps['dist_reverse'].get(user.get('district_id'), 'District')})"]
            elif role == "block": origin_options = ["Block Annual Action Plan", f"Block Meeting ({maps['block_reverse'].get(user.get('block_id'), 'Block')})"]
            else: origin_options = ["District Plan", "Block Plan", "District Meeting", "Block Meeting"]
            ...
            if sel_conv_type == "Technical Convergence (Zero Fund/NOC)": dept_fund = vbg_fund = 0.0; st.info("ℹ️ Technical Convergence selected.")
            else: col_f3, col_f4 = st.columns(2); dept_fund = col_f3.number_input("Department Fund (₹ Lakhs)", min_value=0.0, step=0.1); vbg_fund = col_f4.number_input("VB-G RAM G Fund (₹ Lakhs)", min_value=0.0, step=0.1)
            ...
            if st.button("Commit Activity Registration", type="primary", use_container_width=True):
                errors = []
                if selected_pia == "Select PIA": errors.append("Please select a valid PIA.")
                ...
                if has_add_gp == "Yes" and additional_gp == "Select GP": errors.append("Please choose valid Additional GP Name.")
                ...
                if sel_conv_type != "Technical Convergence (Zero Fund/NOC)" and dept_fund == 0.0 and vbg_fund == 0.0: errors.append("Financial Convergence requires a Fund allocation.")
                if persondays <= 0: errors.append("Expected Persondays must be greater than zero.")
                if scheme_data["convergence"] and not scheme_data["scheme_name"]: errors.append("Scheme name mandatory when Convergence = Yes.")
                if errors:
                    for err in errors: st.error(f"⚠️ {err}")
                else:
                    block_id = maps["block_map"].get(sel_block)
                    geo_string = f"Loc: {inp_loc_details} | GP: {primary_gp}"
                    if has_add_gp == "Yes" and additional_gp and additional_gp != "Select GP": geo_string += f" | Addl GP: {additional_gp} (Portion: {add_gp_portion})"
                    if inp_lat_long: geo_string += f" | GPS: {inp_lat_long}"
                    duplicate_check = supabase.table("convergence_register").select("id").eq("financial_year_id", selected_fy_id).eq("block_id", block_id).eq("department_id", selected_dept_id).eq("activity_description", final_work_name).eq("geo_location", geo_string).execute()
                    if len(duplicate_check.data) > 0: st.error("⛔ Duplicate Work Detected!")
                    else:
                        insert_data = {
                            ...
                            "activity_id": selected_act_record["id"] if selected_act_record else None, # <-- ADDED activity_id for Create mode
    ...
    if len(tabs) > 2:
        with tabs[2]: edit_delete_section(records, maps, supabase, user, master)

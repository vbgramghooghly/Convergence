...
def show():
    ...
    # ======================== 1. MASTER DATA FETCH ========================
    departments = (
        supabase.table("departments").select("id, department_name").execute().data or []
    )
    wings = (
        supabase.table("department_wings")
        .select("id, department_id, wing_name, entity_type")
        .execute()
        .data or []
    )
    blocks_data = (
        supabase.table("blocks").select("id, block_name, district_id").execute().data or []
    )
    ...
    # Build Unified Department / Wing Options for Resolutions
    dept_options = []
    ...
    # Helper function to format Department/Wing display cleanly
    ...
    with tab1:
        ...
        if not df_meetings.empty:
            ...
            disp_df = df_m[
                ["meeting_date", "meeting_type", "Block", "venue", "chairperson"]
            ].copy()
            ...
            detail_sel = st.selectbox(
                ...
                format_func=lambda x: (
                    ...
                    f" | {df_meetings[df_meetings['id'] == x]['meeting_type'].values[0]} Level"
                ),
            )
            ...
            with st.container(border=True):
                ...
                if att_data and isinstance(att_data, list) and len(att_data) > 0:
                    ...
                    disp_att_df = att_df[[
                        "official_name", "official_designation", "official_phone", "official_email",
                        "attended_by_subordinate", "subordinate_name", "subordinate_designation", "subordinate_phone"
                    ]]
                    disp_att_df.columns = [
                        "Official Name", "Official Designation", "Official Phone", "Official Email",
                        "Subordinate Attended?", "Subordinate Name", "Subordinate Designation", "Subordinate Phone"
                    ]

                    def highlight_subs(row):
                        return ["background-color: #FFF3CD"] * len(row) if row["Subordinate Attended?"] else [""] * len(row)
                        ...
                        if e_attendees:
                            ...
                            for cid in e_attendees:
                                ...
                                with st.container(border=True):
                                    ...
                                    is_sub = st.checkbox("Attended by Subordinate/Representative?", value=prev_is_sub, key=f"e_chk_{cid}_{detail_sel}")
            ...
            with st.form("schedule_meeting_form"):
                ...
                if statutory_officials:
                    ...
                    for cid, info in statutory_officials.items():
                        if cols[idx % 2].checkbox(f"{info['name']} - {info['designation']}", key=f"stat_{cid}"):
                            selected_contact_ids.append(cid)
                        idx += 1
                else:
                    st.warning("No statutory members found in the directory for this jurisdiction.")
    ...
    with tab3:
        ...
        if df_meetings.empty:
            ...
            # --- A. MARK ACTUAL ATTENDANCE (FIXED UI) ---
            with st.expander("👥 A. Mark Actual Attendance", expanded=True):
                curr_att = proc_meeting_data.get("attendees") or []
                if not isinstance(curr_att, list):
                    curr_att = []

                # Removed st.form to allow interactive expansion of subordinate text inputs
                with st.container():
            ...
            with st.expander("⏳ B. Review Past Decisions & Progress", expanded=False):
                ...
                if past_ap_query:
                    ...
                    if not df_past.empty:
                        df_past["Department"] = df_past.apply(format_dept_display, axis=1)
                        
                        st.dataframe(
                            df_past[["Department", "action_point", "responsible_officer", "status", "remarks"]],
                            use_container_width=True, hide_index=True,
                        )

                        with st.form("quick_update_form"):
                            col_u1, col_u2 = st.columns(2)
                            u_id = col_u1.selectbox("Select Resolution", df_past["id"].tolist(), format_func=lambda x: f"{df_past[df_past['id'] == x]['action_point'].values[0][:40]}...")
            ...
            with st.expander("📝 C. General Minutes & New Resolutions", expanded=True):
                ...
                if not current_att_data:
                    ...
                    with st.form("add_new_resolution"):
                        ...
                        res_dept_label = col_r1.selectbox("Converging Department / Wing*", dept_labels)
                        selected_opt = next(opt for opt in dept_options if opt['label'] == res_dept_label)
    ...
    with tab4:
        ...
        if df_meetings.empty:
            ...
            if ap_data:
                ...
                if not df_ap.empty:
                    df_ap["Department"] = df_ap.apply(format_dept_display, axis=1)
        ...
        if report_type == "By Specific Meeting (Chairperson Report)":
            if not df_meetings.empty:
                ...
                if mtg_ap:
                    ...
                    df_rep_ap["Department"] = df_rep_ap.apply(format_dept_display, axis=1)

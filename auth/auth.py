...
def check_password():
  ...
  with col_left:
    ...
    st.markdown(
        "<h1 class='portal-title'>📊 VB-G RAM G Convergence</h1>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<p class='portal-subtitle'>Unified District, Block, and Department"
        " Level Convergence Management Portal</p>",
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        "<div class='feature-item'>📈 <b>Real-Time Progress Tracking:</b>"
        " Monitor physical & financial achievements instantly.</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='feature-item'>🏛️ <b>Multi-Tier Governance:</b>"
        " Role-based access for Districts, Blocks, and Departments.</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='feature-item'>📊 <b>Dynamic Visualizations:</b>"
        " Automated analytics, dashboards, and instant Excel/PDF reporting.</div>",
        unsafe_allow_html=True,
    )
    st.markdown(
        "<div class='feature-item'>🔒 <b>Secure & Transparent:</b> Complete"
        " audit logging and streamlined activity registers.</div>",
        unsafe_allow_html=True,
    )
  ...
  with col_right:
    ...
    st.markdown(
        "<p style='color: #718096; font-size: 0.9rem; margin-bottom:"
        " 15px;'>Enter your credentials to access your workspace.</p>",
        unsafe_allow_html=True,
    )

    with st.form("login_form"):
      username = st.text_input(
          "Username / Email", placeholder="Enter your username"
      )
      ...
      submit_btn = st.form_submit_button(
          "Sign In to Workspace", type="primary", use_container_width=True
      )
        ...
        if not username or not password:
          ...
          try:
            ...
            login_email = (
                clean_input
                if "@" in clean_input
                else f"{clean_input}@hooghly.gov.in"
            )
            ...
            if not users_data:
              ...
              if not user_profile.get("active", True):
                st.error(
                    "🚫 This account has been deactivated. Contact Superadmin."
                )
              else:
